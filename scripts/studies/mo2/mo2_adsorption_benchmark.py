...
Single site only, same choice as scripts/studies/mo2/o_adsorption_benchmark.py and for the same reason:
...
--formulas <FORMULA>); an oxide with no local structure is skipped, not fatal.
    ...
    python scripts/studies/mo2/mo2_adsorption_benchmark.py runs/mo2_pilot \\
...
# Walk up to the repo root by marker rather than counting parents: this file has already
# been moved once (scripts/ -> scripts/ClaudeScripts/ -> scripts/studies/mo2/) and a
# hardcoded parent count silently resolves to the wrong directory every time it moves.
REPO = next(d for d in Path(__file__).resolve().parents if (d / "pyproject.toml").exists())
LIT_CSV = REPO / "data" / "ClaudeArtifacts" / \
